...
contract VendaDeSalas is owned {
    ...
    function comprarSala () payable public  {
        ...
        require (balanceOf[msg.sender] < 1); //Each buyer can only acquire 1 room
        balanceOf[msg.sender] = totalSupply / 10;
        emit Transfer(address(0), msg.sender, totalSupply / 10);  //Gives 1 room to buyer
    }

function coletarPagamentos () public onlyowner {  //Sends all contract ether to owner
    require (msg.sender == owner);
    owner.transfer((address(this)).balance);
    require (dividendPerToken == 0); //Requires that all dividends have been paid
}
